...
FluidRenderer::FluidRenderer() :
    ...
    containerOpacity(DEFAULT_PLANE_OPACITY),
    showEnvMap(true),
...
void FluidRenderer::visualizeCubicContainer(Camera* camera, FluidContainer* container, bool drawAsOutline) {
    ...
    glm::vec3 position = container->getCurrentPosition();
    glm::vec3 scale = container->getCurrentScale() * 2.0f;
    glm::mat3 rotation = container->getCurrentBasis();

    glm::mat4 model(1.0f);
    model[0] = glm::vec4(rotation[0] * scale.x * renderScale, 0.0f);
    model[1] = glm::vec4(rotation[1] * scale.y * renderScale, 0.0f);
    model[2] = glm::vec4(rotation[2] * scale.z * renderScale, 0.0f);
    model[3] = glm::vec4(position * renderScale, 1.0f);

    cubeRenderer.draw(camera, containerOpacity, model, drawAsOutline);
