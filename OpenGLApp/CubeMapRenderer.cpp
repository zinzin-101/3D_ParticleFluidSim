...
bool CubeMapRenderer::init(const std::string& texturePath, bool useExternalShader) {
    ...
    // rendering function for cube map
    glDepthFunc(GL_LEQUAL);
    glEnable(GL_TEXTURE_CUBE_MAP_SEAMLESS);

    // frame buffer for creating cubemap
    ...
    int height{};
    int nrComponents{};
    float* data = stbi_loadf(FileSystem::getPath(texturePath).c_str(), &width, &height, &nrComponents, 0);
    ...
    equirectangularToCubeMapShader.setMat4("model", glm::mat4(1.0f));
